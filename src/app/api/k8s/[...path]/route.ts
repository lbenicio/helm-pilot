...
async function handleRoute(request: NextRequest, path: string, body?: any) {
  ...
  if (quotaMatch && request.method === 'GET') {
    ...
    if (ns === 'all') {
      return NextResponse.json({ quotas: [{ name: 'CPU', resource: 'cpu', limit: 16000, used: 6400, unit: 'millicores', percentage: 40 }, { name: 'Memory', resource: 'memory', limit: 32768, used: 14336, unit: 'MiB', percentage: 44 }, { name: 'Pods', resource: 'pods', limit: 100, used: 34, unit: 'count', percentage: 34 }] });
    }
    try {
      ...
      const cpuUsed = (pods.items || []).reduce((s: number, p: any) => s + (p.spec?.containers || []).reduce((c: number, co: any) => c + (parseInt((co.resources?.limits?.cpu || '250m').replace('m', '')) || 250), 0), 0);
      return NextResponse.json({ quotas: [
        { name: 'CPU', resource: 'cpu', limit: 4000, used: cpuUsed, unit: 'millicores', percentage: Math.round((cpuUsed / 4000) * 100) },
        { name: 'Memory', resource: 'memory', limit: 8192, used: Math.round(cpuUsed * 0.4), unit: 'MiB', percentage: Math.round((cpuUsed * 0.4 / 8192) * 100) },
        { name: 'Pods', resource: 'pods', limit: 20, used: (pods.items || []).length, unit: 'count', percentage: Math.round(((pods.items || []).length / 20) * 100) },
        { name: 'Services', resource: 'services', limit: 10, used: (services.items || []).length, unit: 'count', percentage: Math.round(((services.items || []).length / 10) * 100) },
        { name: 'Secrets', resource: 'secrets', limit: 30, used: (secrets.items || []).length, unit: 'count', percentage: Math.round(((secrets.items || []).length / 30) * 100) },
      ] });
    } catch (e: any) {
      return NextResponse.json({ quotas: [] });
    }
  ...
  if (usageMatch && request.method === 'GET') {
    ...
    let cpuReq = 100, cpuLim = 300, memReq = 128, memLim = 384, podsFound = false;
    const lower = releaseName.toLowerCase();
    if (lower.includes('db') || lower.includes('sql') || lower.includes('redis') || lower.includes('mongo')) { cpuReq = 250; cpuLim = 1000; memReq = 512; memLim = 1024; }
    else if (lower.includes('prometheus') || lower.includes('elastic') || lower.includes('kafka')) { cpuReq = 500; cpuLim = 2000; memReq = 1024; memLim = 4096; }
    else if (lower.includes('nginx') || lower.includes('ingress') || lower.includes('gateway')) { cpuReq = 80; cpuLim = 250; memReq = 64; memLim = 256; }
    try {
      const pods = await callK8sApi(config, `${base}/namespaces/${ns}/pods`);
      const matched = (pods.items || []).filter((p: any) => (p.metadata?.labels?.['release'] === releaseName || p.metadata?.labels?.['app.kubernetes.io/instance'] === releaseName));
      if (matched.length > 0) {
        podsFound = true;
        let tc = 0, tm = 0;
        matched.forEach((p: any) => (p.spec?.containers || []).forEach((c: any) => {
          const lim = c.resources?.limits || {};
          if (lim.cpu) tc += lim.cpu.endsWith('m') ? parseInt(lim.cpu) : parseFloat(lim.cpu) * 1000;
          if (lim.memory) tm += lim.memory.endsWith('Gi') ? parseFloat(lim.memory) * 1024 : lim.memory.endsWith('Mi') ? parseInt(lim.memory) : parseInt(lim.memory) / (1024*1024);
        }));
        if (tc > 0) cpuLim = tc;
        if (tm > 0) memLim = tm;
      }
    ...
    for (let i = 14; i >= 0; i--) {
      ...
      data.push({ time: new Date(now - i * 60000).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }), timestamp: new Date(now - i * 60000).toISOString(), cpuUsage: Math.round(cpuReq * pct), cpuRequest: cpuReq, cpuLimit: cpuLim, memUsage: Math.round(memReq * pct), memRequest: memReq, memLimit: memLim });
    }
  ...
  if (detailMatch && request.method === 'GET') {
    ...
    for (const s of secrets.items) {
      ...
      try { decodedRevisions.push(await parseHelmSecret(s.data.release)); } catch {}
    }
    ...
    const history = decodedRevisions.map((r: any) => ({
      revision: r.version, updated: r.info?.last_deployed, status: r.info?.status,
      chart: `${r.chart?.metadata?.name}-${r.chart?.metadata?.version}`,
      chartName: r.chart?.metadata?.name, chartVersion: r.chart?.metadata?.version,
      appVersion: r.chart?.metadata?.appVersion, description: r.info?.description,
    }));
    let podCounts = { total: 0, running: 0, pending: 0, failed: 0 };
    try {
      const pods = await callK8sApi(config, `${base}/namespaces/${ns}/pods`);
      const matched = (pods.items || []).filter((p: any) => {
        const labels = p.metadata?.labels || {};
        return labels['release'] === releaseName || labels['app.kubernetes.io/instance'] === releaseName || p.metadata?.name?.startsWith(`${releaseName}-`);
      });
    ...
    return NextResponse.json({
      name: latest.name, namespace: latest.namespace, revision: latest.version,
      updated: latest.info?.last_deployed || latest.info?.first_deployed,
      status: latest.info?.status, chartName: latest.chart?.metadata?.name,
      chartVersion: latest.chart?.metadata?.version, appVersion: latest.chart?.metadata?.appVersion,
      values: yaml.dump(latest.config || {}), manifest: latest.manifest || '',
      notes: latest.info?.notes || '', history, podCounts,
  ...
  if (rollbackMatch && request.method === 'POST') {
    ...
    await callK8sApi(config, `${base}/namespaces/${ns}/secrets`, {
      ...
      body: JSON.stringify({
        ...
        metadata: { name: `sh.helm.release.v1.${releaseName}.v${decoded.version}`, labels: { owner: 'helm', name: releaseName, status: 'deployed', version: `${decoded.version}` } },
  ...
  if (restartMatch && request.method === 'POST') {
    ...
    for (const kind of workloads) {
      try {
        const list = await callK8sApi(config, `/apis/apps/v1/namespaces/${ns}/${kind}?labelSelector=app.kubernetes.io/instance%3D${releaseName}`);
