...
export default function ReleaseDetails({ name, namespace, activeCluster, onClose, onRefresh }: ReleaseDetailsProps) {
  ...
  const handleUninstall = async () => {
    if (
      !confirm(
        `CRITICAL ACTION: Are you sure you want to completely uninstall and delete "${name}"? This deletes all history and resources.`,
      )
    )
      return;
        ...
        <div className="flex items-start gap-3.5">
          ...
          <div>
            <div className="flex flex-wrap items-center gap-2 mb-1">
              ...
              <span
                className={`inline-flex items-center px-2 py-0.5 rounded text-[10px] font-semibold uppercase ${
                  release.status === 'deployed' ? 'bg-emerald-100 text-emerald-800' : 'bg-rose-100 text-rose-800'
                }`}
              >
                {release.status}
              </span>
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400">
              Namespace: <b className="text-slate-700 dark:text-slate-200">{release.namespace}</b> &bull; Chart:{' '}
      ...
      <div className="flex border-b border-[#E1E4E8] dark:border-slate-700 px-6 bg-slate-50 dark:bg-slate-800/50 overflow-x-auto">
        {(['overview', 'history', 'values', 'manifest', 'security'] as const).map((tab) => (
          ...
          >
            {tab === 'security' && (
              <Shield
                className={`w-3.5 h-3.5 ${activeTab === 'security' ? 'text-blue-600 dark:text-blue-400 animate-pulse' : 'text-slate-400'}`}
              />
            )}
            <span>{tab === 'values' ? 'values.yaml' : tab === 'security' ? 'Antivírus Scan' : tab}</span>
          </button>
        ))}
      </div>
      ...
      <div className="p-6 overflow-y-auto max-h-[60vh]">
        <AnimatePresence mode="wait">
          ...
          >
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-3.5">
                  <div className="bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 p-3.5 rounded-lg text-center">
                    <span className="block text-[10px] text-slate-500 dark:text-slate-400 uppercase font-semibold mb-1">
                      Active Revision
                    </span>
              ...
              <div className="space-y-4">
                ...
                {(() => {
                  const filteredHistory = (release?.history || [])
                    ...
                    .sort((a: any, b: any) => {
                      return historySort === 'desc' ? b.revision - a.revision : a.revision - b.revision;
                    });
                  ...
                  if (filteredHistory.length === 0) {
                    return (
                      <div className="text-center py-12 border border-dashed border-slate-200 dark:border-slate-800 rounded-xl space-y-2">
                        ...
                        <button onClick={() => setHistorySearch('')} className="text-xs text-blue-600 font-semibold hover:underline">
                          Clear search filter
                        </button>
                      </div>
                    );
                      ...
                      {filteredHistory.map((rev: any) => {
                        ...
                        return (
                          <div key={rev.revision} className="relative group">
                            {/* Timeline Bullet */}
                            <div
                              className={`absolute -left-7.75 top-4 w-3.5 h-3.5 rounded-full border-2 transition-all ${
                                isCurrentActive
                            ...
                            {/* Revision Card */}
                            <div
                              className={`border rounded-xl shadow-sm transition overflow-hidden bg-white dark:bg-slate-900 ${
                                isCurrentActive
                                      ...
                                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
                                        ...
                                        <div className="space-y-3 bg-white dark:bg-slate-900 p-3 rounded-lg border border-slate-100 dark:border-slate-800">
                                          ...
                                          <div className="grid grid-cols-2 gap-2 text-[11px]">
                                            ...
                                            <div className="font-bold text-slate-800 dark:text-slate-200 font-mono">v{rev.revision}</div>
            ...
            {activeTab === 'values' && (
              <div className="space-y-4">
                <div className="flex items-center justify-between border-b border-[#E1E4E8] dark:border-slate-700 pb-2">
                  ...
                  >
                    {upgrading ? <RefreshCw className="w-3.5 h-3.5 animate-spin" /> : <Save className="w-3.5 h-3.5" />}
                    Upgrade Release
                  </button>
                </div>
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between border-b border-[#E1E4E8] dark:border-slate-700 pb-2">
                  <span className="text-xs text-slate-500 dark:text-slate-400">
                    Compiled Kubernetes resource manifests compiled by Helm template.
                  </span>
            ...
            {activeTab === 'security' && <AntivirusScanner manifest={release?.manifest || ''} releaseName={name} />}
          </motion.div>
        </AnimatePresence>
      </div>
    </motion.div>
