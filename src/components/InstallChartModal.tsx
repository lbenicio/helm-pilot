    ...
    <div className="fixed inset-0 bg-slate-950/40 backdrop-blur-sm flex items-center justify-center z-50 p-4 font-sans select-none">
      <div className="bg-white dark:bg-slate-900 border border-[#E1E4E8] dark:border-slate-700 rounded-xl w-full max-w-3xl max-h-[90vh] overflow-hidden flex flex-col shadow-xl">
        ...
        <div className="flex items-center justify-between border-b border-[#E1E4E8] dark:border-slate-700 p-5 bg-slate-50/50 dark:bg-slate-800/50">
          ...
          <button
            onClick={onClose}
            className="p-1.5 hover:bg-slate-200 dark:hover:bg-slate-700 rounded-lg text-slate-400 transition cursor-pointer"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
          ...
          <div className="flex-1 overflow-y-auto p-6">
            {step === 'basic' ? (
              ...
              <div className="space-y-5 max-w-lg mx-auto">
                <div>
                  <label className="block text-xs text-slate-500 dark:text-slate-400 font-bold uppercase tracking-wider mb-1.5">
                    Release Name
                  </label>
                  <input
                    type="text"
                ...
                <div className="bg-blue-50 dark:bg-blue-950/20 border border-blue-100 dark:border-blue-900/30 rounded-lg p-4">
                  <p className="text-xs text-blue-700 dark:text-blue-300">
                    Ready to deploy with defaults. Click <b>Advanced</b> to customize values.yaml.
                  </p>
                </div>
              </div>
            ) : (
            ...
            {step === 'basic' ? (
              <>
                <button
                  ...
                  onClick={onClose}
                  disabled={deploying}
                  className="px-4 py-2 text-sm text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200 transition cursor-pointer"
                >
                  Cancel
                </button>
                <button
              ...
              <>
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    onClick={onClose}
                    disabled={deploying}
                    className="px-4 py-2 bg-white dark:bg-slate-900 border border-[#E1E4E8] dark:border-slate-700 text-sm font-semibold text-slate-600 dark:text-slate-300 rounded-lg cursor-pointer"
                  >
                    Cancel
                  </button>
                  <button
