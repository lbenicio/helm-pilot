...
import { Activity, ChevronRight, Copy, Folder, Heart, LayoutDashboard, Library, LogOut, Moon, Search, Ship, Sun, X } from 'lucide-react';
        ...
        <div className="px-6 md:px-8 h-16 flex items-center justify-between">
          ...
          <nav className="hidden md:flex items-center gap-1.5 bg-slate-100 dark:bg-slate-800 p-1 border border-[#E1E4E8] dark:border-slate-700 rounded-xl">
            ...
            <button
              ...
              className={`flex items-center gap-1.5 px-3.5 py-1.5 rounded-lg text-xs font-semibold transition cursor-pointer ${isEvents ? 'bg-blue-600 text-white shadow-sm' : 'text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white hover:bg-slate-200/50 dark:hover:bg-slate-700/50'}`}
            >
            ...
            <button
              ...
              className={`flex items-center gap-1.5 px-3.5 py-1.5 rounded-lg text-xs font-semibold transition cursor-pointer ${isHealth ? 'bg-blue-600 text-white shadow-sm' : 'text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white hover:bg-slate-200/50 dark:hover:bg-slate-700/50'}`}
            >
          ...
          <div className="hidden md:flex items-center relative flex-1 max-w-xs lg:max-w-sm xl:max-w-md mx-4">
            ...
            <input
              ...
              onKeyDown={(e) => {
                if (e.key === 'Enter' && globalSearchQuery) router.push(`/search?q=${encodeURIComponent(globalSearchQuery)}`);
              }}
            ...
            <>
              ...
              >
                ...
                <span className="flex items-center gap-1">
                  Namespace: <b className="font-mono">{releaseNs}</b>
                </span>
              </motion.button>
              <ChevronRight className="w-3.5 h-3.5 text-slate-300 dark:text-slate-600" />
              <span className="text-slate-800 dark:text-slate-200 bg-slate-100 dark:bg-slate-800 px-2 py-1 rounded-md font-bold font-mono">
                {releaseName}
              </span>
            </>
          ) : isCharts ? (
            ...
            <span className="text-slate-800 dark:text-slate-200 bg-slate-100 dark:bg-slate-800 px-2 py-1 rounded-md font-bold flex items-center gap-1.5">
              ...
              <span className="flex items-center gap-1">
                {selectedNamespace === 'all' ? (
                  ...
                  <>
                    Namespace: <span className="font-mono font-bold">{selectedNamespace}</span>
                  </>
                )}
              </span>
            </span>
